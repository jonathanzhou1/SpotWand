...
def query_arm_cartesian_trajectory(t):
    ...
    # Draw a circle
    R = 0.25  # circle radius in meters --> increase and decrease for size of flower
    T = 16.0  # period to go all the way around circle. Longer T = slower
    k = 5 #number of petals --> increasing this may cause sharper turns
    theta = 2 * math.pi * t / T
    x = R * math.cos(k * theta) * math.cos(theta)
    y = R * math.cos(k * theta) * math.sin(theta)
    z = 0.0
    quat = Quat(0.7071068, 0.7071068, 0, 0) #originallt Quat(1, 0, 0, 0)

    #Velocities
    dtheta_dt = 2 * math.pi / T
    vx = -R * (k * math.sin(k * theta) * math.cos(theta) + math.cos(k * theta) * math.sin(theta)) * dtheta_dt
    vy = R * (-k * math.sin(k * theta) * math.sin(theta) + math.cos(k * theta) * math.cos(theta)) * dtheta_dt
    vz = 0
    return SE3Pose(x, y, z, quat), SE3Velocity(vx, vy, vz, 0, 0, 0)

def plot_trajectory(run_time, dt):
    """
    Plot the trajectory generated by query_arm_cartesian_trajectory.
    """
    times = [i * dt for i in range(int(run_time / dt))]
    trajectory_points = [query_arm_cartesian_trajectory(t)[0] for t in times]
...
def main():
    ...
    if options.simulate:
        ...
        plot_trajectory(run_time=RUN_TIME, dt=0.05)
    else:
