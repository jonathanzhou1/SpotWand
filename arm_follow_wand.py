...
#JZ: import the csv package to read our wand data
import pandas as pd
import numpy as np
...
def parse_csv():
    #Get all the rows
    df = pd.read_csv("wand_log.csv", header=None) #assume cols 0-2 = X, Y, Z

    cols = df.columns
    #Center x and y columns around 0
    df[cols[0]] = df[cols[0]] - df[cols[0]].mean()
    df[cols[1]] = df[cols[1]] - df[cols[1]].mean()
...
def query_arm_position(t, rows):
    """
    Given a time t in the trajectory, return the SE3Pose and SE3Velocity at this point in the trajectory
    """

    #Get the current row's positions
    curRowIdx = int(t * 100)
    curRow = rows[curRowIdx]
    x = float(curRow[0]) / 1.5
    y = float(curRow[1]) / 1.5
    z = 0
    quat = Quat(1, 0, 0, 0)

    #calculate the slope for velocities
    #if prev and next rows exist, do slope from prev -> next
    dx = 0
    dy = 0
    dz = 0
    dt = 0.4
    prevIdx = curRowIdx - 20
    nextIdx = curRowIdx + 20
    
    if prevIdx >= 0 and nextIdx < len(rows):
        #in middle of dataset
        prevRow = rows[prevIdx]
        nextRow = rows[nextIdx]
        dx = (nextRow[0] - prevRow[0]) / dt
        dy = (nextRow[1] - prevRow[1]) / dt
    elif prevIdx >= 0:
        #at end
        prevRow = rows[prevIdx]
        dt = 0.2
        dx = (curRow[0] - prevRow[0]) / dt
        dy = (curRow[1] - prevRow[1]) / dt
    elif nextIdx < len(rows):
        #at beginning
        nextRow = rows[prevIdx]
        dt = 0.2
        dx = (nextRow[0] - curRow[0]) / dt
        dy = (nextRow[1] - curRow[1]) / dt

    return SE3Pose(x, y, z, quat), SE3Velocity(dx, dy, dz, 0, 0, 0)

def plot_trajectory(run_time, dt):
    """
    Plot the trajectory generated by query_arm_position.
    """
    rows = parse_csv()
    times = [i * dt for i in range(int(run_time / dt))]
    trajectory_points = [query_arm_position(t, rows)[0] for t in times]
    x_vals = [point.x for point in trajectory_points]
    y_vals = [point.y for point in trajectory_points]
    
    plt.figure(figsize=(8, 8))
    plt.plot(x_vals, y_vals, label="Rose Curve Trajectory")
    plt.xlabel("X Position (m)")
    plt.ylabel("Y Position (m)")
    plt.title("Simulated Rose Curve Trajectory for Spot Arm")
    plt.legend()
    plt.grid()
    plt.axis('equal')
    plt.show()
...
def arm_trajectory(config):
    ...
    with bosdyn.client.lease.LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        #JZ: get the rows from the csv data!
        rows = parse_csv()
        while time.time() - start_time < RUN_TIME:
            ...
            for i in range(N):
                ...
                pos, vel = query_arm_position(t, rows)
            ...
            segment_end_time = segment_start_time + dt * N
...
def main():
    ...
    bosdyn.client.util.add_base_arguments(parser) #comment out if simulation mode
    parser.add_argument("--simulate", action="store_true", 
                        help="Simulate and plot the trajectory without connecting to the robot.")
    parser.add_argument("--hostname", help="Hostname or IP address of Spot robot.", required=False)
    
    options = parser.parse_args()

    #running simulation: python arm_rose_curve.py --simulate
    #running with arm: python arm_rose_curve.py --hostname <robot_hostname>

    if not options.simulate and not options.hostname:
        parser.error("the following arguments are required: --hostname when not using --simulate")

    if options.simulate:
        # Plot the trajectory without connecting to the robot
        plot_trajectory(run_time=RUN_TIME, dt=0.2)
    else:
        # Run the actual arm trajectory on the robot
